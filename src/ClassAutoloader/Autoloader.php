 ...
 *
 * Оглавление:
 * @see Autoloader::$autoMockerEnabled - Ключ, нужно ли все загружаемые классы превращать в мок-классы или нет
 * @see Autoloader::getPath() - Вернет путь к загружаемому классу
 * @see Autoloader::loadOrException() - Загружает класс, в случае провала выбрасывает исключение
 * @see Autoloader::load() - Загружает класс, вернет успех или провал загрузки
 * --- Прочее
 * @see Autoloader::$autoloaderDriver - Драйвер "базового автозагрузчика проекта"
 * @see Autoloader::$autoloaderFilter - Фильтр, для определения, нужно ли преобразовывать загружаемый класс в мок-класс
 * @see Autoloader::register() - Осущеставляет регистрацию автозагрузчика
 * @see Autoloader::unregister() - Снимает регистрацию автозагрузчика
 * --- Управление автозагрузчиками
 * @see Autoloader::$autoloaderList - Список всех созданных объектов-автозагрузчиков
 * @see Autoloader::$allConvertToMock - Принудительные правила преобразования в мок-классы
 */
class Autoloader
{
    /**
    ...
    /**
     * Принудительные правила преобразования в мок-классы
     *
     ...
     */
    public function __construct(AutoloaderDriverInterface $autoloaderDriver, AutoloaderFilterInterface $autoloaderFilter)
    {
    ...
    protected function executingClassLoad(string $class, bool $ifFailToException): bool
    {
        ...
        {
            return $this->executingClassLoadCreateMock($classPath, $ifFailToException);
        }
        else
        {
     ...
     */
    protected function executingClassLoadCreateMock(string $classPath, bool $ifFailToException): bool
    {
        if (!is_file($classPath))
        {
            if ($ifFailToException) throw new PhpMockerAutoloaderPathIsNotFileException($classPath);
            else return false;
        }
        elseif (!is_readable($classPath))
        {
            if ($ifFailToException) throw new PhpMockerAutoloaderPathIsNotReadableException($classPath);
            else return false;
        }
        ...
        try {
            HardMocker::createForCode(
                file_get_contents($classPath)
            );

            return true;
        }
        // Попытка загрузить интерфейс должна проваливаться, так как нет смысла превращать интерфейс в мок-класс
        catch (HardMockerCreateForInterface)
        {
            require_once($classPath);
            return true;
        }
        ...
        {
            if ($ifFailToException) throw $error;
            else return false;
        }
    }
}
