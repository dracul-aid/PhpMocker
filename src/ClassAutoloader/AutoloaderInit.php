 ...
 *
 * Оглавление:
 * @see AutoloaderInit::create() - Создаст и зарегестрирует автозагрузчик
 * --- Установка настроек
 * @see AutoloaderInit::setComposerVendorPath() - Установит путь к каталогу vendor композера (функция не проверяет путь на корректность)
 * @see AutoloaderInit::searchAndSetComposerVendorPath() - Пытается найти путь к папке vendor композера
 * @see AutoloaderInit::setDriverAutoloaderUnregister() - Нужно ли "выключить" автозагрузку с помощью стандартного автозагрузчика проекта
 * @see AutoloaderInit::setAutoloaderDriver() - Устанавливает драйвер для поиска путей к вызываемому классу, для автозагрузчика
 * @see AutoloaderInit::setAutoloaderFilter() - Устанавливает фильтр, для проверки, нужно ли преобразовывать загружаемый класс, в мок-класс
 */
class AutoloaderInit
{
    ...
    public function create(bool $register = true, bool $prepend = false): Autoloader
    {
        ...
        $autoloader = new Autoloader($this->autoloaderDriver, $this->autoloaderFilter);
