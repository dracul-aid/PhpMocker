 ...
 *
 * Оглавление:
 * @see ObjectManager::$toObject - Объект для которого создан менеджер
 * @see ObjectManager::getManager() - Вернет "менеджер мок-объекта" для мок-объекта
 * --- Свойства мок-класса
 * @see ObjectManager::getClassManager() - Вернет менеджер мок-класса, которому принадлежит мок-объект
 * @see ObjectManager::getDriver() - Вернет имя класса, с помощью которого был создан мок-класс
 * @see ObjectManager::getToClass() - Вернет имя мок-класса, для которого создан менеджер
 * @see ObjectManager::getToClassScheme() - Вернет схему класса, для которого создан менеджер (если ее нет, создаст ее)
 * --- Работа с мок-методами
 * @see ObjectManager::$mockMethodNames - Список имен методов класса, для которых можно получить "мок-метод"
 * @see ObjectManager::$methodManagers - Массив с менеджерами мок-методов
 * @see ObjectManager::getMethodManager() - Вернет менеджер мок-метода
 * --- Взаимодействие с элементами (в том числе и protected и private)
 * @see ObjectManager::getProperty() - Получение значения свойства
 * @see ObjectManager::setProperty() - Установка значения свойства
 * @see ObjectManager::callMethod() - Вызов метода
 */
class ObjectManager extends AbstractClassAndObjectManager
{
    ...
    {
        return [$this->toObject, Tools::methodPropertyGet($this->getClassManager()->index)]($name);
    }
    ...
    public function setProperty(string|array $nameOrList, mixed $value = null): self
    {
        ...
        {
            [$this->toObject, Tools::methodPropertySet($this->getClassManager()->index)]($nameOrList, $value);
        }
    ...
    {
        return [$this->toObject, Tools::methodCall($this->getClassManager()->index)]($name, $arguments);
    }
