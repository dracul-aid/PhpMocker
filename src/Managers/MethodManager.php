 ...
 *
 * Оглавление:
 * @see MethodManager::$ownerManager - Объект-менеджер - владелец менеджера метода
 * @see MethodManager::$name - Имя метода
 * @see MethodManager::call() - Вызов метода (в том числе и protected и private)
 * @see MethodManager::clearCases() Сбрасывает все "кейсы вызова" и счетчики вызовов
 * --- Счетчики вызовов мок-метода
 * @see MethodManager::$countCall - Общий счетчик числа вызовов метода
 * @see MethodManager::$countCallWithoutCases - Счетчик вызовов без отработанных кейсов вызова
 * @see MethodManager::$countCallUserFunctionReturn - Счетчик вызовов "Функции, которая будет выполнена перед выполнением основного тела метода" с перехватом выполнения основного кода
 * --- Кейсы вызова
 * @see MethodManager::$cases - Массив со списком всех кейсов вызовов
 * @see MethodManager::$userFunction - Функция, которая будет выполнена перед выполнением основного тела метода (только, если метод может быть мок-методом)
 * @see MethodManager::setUserFunction() - Установит пользовательскую функцию, предваряющую выполнение мок-метода
 * @see MethodManager::case() - Вернет кейс вызова метода (если нет, создаст его)
 * @see MethodManager::defaultCase() - Вернет кейс вызова метода "по умолчанию"
 * --- Прочее
 * @see MethodManager::hasCalled() - Отрабатывает вызов метода для данного кейса-вызова
 */
class MethodManager
{
    /**
     ...
     */
    private function caseIndex(array $arguments): string
    {
