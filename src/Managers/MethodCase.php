 ...
 * --- Свойства кейса
 * @see MethodCase::$methodManager - Объект менеджер мок-метода, которому принадлежит кейс вызова
 * @see MethodCase::$index - Хэш от аргументов вызова
 * @see MethodCase::$arguments - Аргументы вызова кейса
 * @see MethodCase::isWork() - Вернет указание, что кейс вызова может быть обработан
 * --- Счетчики вызова кейса
 * @see MethodCase::$countCall - Счетчик срабатываний кейса (может быть сброшен)
 * @see MethodCase::$countAllCall - Счетчик срабатываний кейса (не может быть сброшен)
 * --- Пользовательская функция, предваряющая выполнение кейса
 * @see MethodCase::$userFunction - Функция, которая будет выполнена перед выполнением основного тела метода
 * @see MethodCase::setUserFunction() - Установит функцию, которая будет выполнена перед выполнением кейса-вызова
 * --- Возвращаемое кейсом значение
 * @see MethodCase::$isCanReturn - Указание, что кейс вызова должен вернуть результат
 * @see MethodCase::$canReturnData - Какие данные должен вернуть мок-метод
 * @see MethodCase::setWillReturn() - Установит ответ для функции, для данного кейса
 * --- Выброс исключения при вызове метода
 * @see MethodCase::$canReturnException - Исключение, которое должно быть выброшено при обращении к мок-методу
 * @see MethodCase::setWillException() - Установит объект-исключение, которое должно будет быть выброшено при вызове метода
 * --- Перезапись значений аргументов метода
 * @see MethodCase::$rewriteArguments - Массив, с значениями для перезаписи аргументов метода
 * @see MethodCase::setRewriteArguments() - Установит значения для перезаписи аргументов метода
 * --- Прочее
 * @see MethodCase::setWillReturnClear() - Отменит установленный ответ для функции (т.е. функция для указанных аргументов будет работать стандартно)
 *
